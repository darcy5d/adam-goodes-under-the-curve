...
class MLTrainingPipeline:
    ...
    def __init__(self, feature_blacklist=None, model_suffix: str = ""):
    ...
    def _prepare_features_and_targets(self):
        ...
        if self.feature_blacklist:
            ...
            feature_cols = [c for c in feature_cols if c not in self.feature_blacklist]
            removed = before - len(feature_cols)
            print(f"Pruning features: removed {removed} by blacklist; remaining {len(feature_cols)}")
    ...
    def implement_model_1_traditional_ml(self):
        ...
        self._generate_predictions(f'traditional_ml{self.model_suffix}', winner_model, margin_model)
    ...
    def implement_model_2_ensemble_ml(self):
        ...
        # Generate predictions
        self._generate_predictions(f'ensemble_ml{self.model_suffix}', winner_model, margin_model)
    ...
    def implement_model_3_deep_learning(self):
        ...
        # Generate predictions
        self._generate_predictions(f'deep_learning{self.model_suffix}', winner_model, margin_model, feature_scaler)
    ...
    def persist_best_model(self):
        """Persist the selected best model (ensemble) and any required artifacts."""
        label = f"ensemble_ml{self.model_suffix}" if self.model_suffix else "ensemble_ml"
        print(f"\nPersisting best model ({label})...")

        # Ensure models directory exists
        os.makedirs('outputs/data/ml_models', exist_ok=True)

        # Select ensemble as production default
        if 'ensemble_ml' not in self.models:
            raise RuntimeError("Ensemble model not trained; cannot persist. Aborting.")

        model_bundle = {
            'winner_model': self.models['ensemble_ml']['winner_model'],
            'margin_model': self.models['ensemble_ml']['margin_model'],
            'type': self.models['ensemble_ml']['type'],
            'label_encoder_winner': self.scalers['winner'],
            'margin_scaler': self.scalers['margin'],
            'feature_columns': getattr(self, 'feature_columns', None)
        }

        output_path = 'outputs/data/ml_models/ensemble_ml_model.pkl' if not self.model_suffix else f'outputs/data/ml_models/ensemble_ml_model{self.model_suffix}.pkl'
        ...
        metadata = {
            'model_name': label,
            'artifacts': list(model_bundle.keys()),
            'feature_count': len(model_bundle['feature_columns']) if model_bundle['feature_columns'] else None
        }
        meta_path = 'outputs/data/ml_models/model_metadata.json' if not self.model_suffix else f'outputs/data/ml_models/model_metadata{self.model_suffix}.json'
        with open(meta_path, 'w') as f:
            json.dump(metadata, f, indent=2)

        print(f"Model persisted to {output_path}")
